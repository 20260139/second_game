...
class Boss3(Boss):
    ...
    def _pick_next(self):
        # RAIN(낙탄 장판)/BLINK(위치이동) 비중↑, RING/CLONE 탄막 비중↓
        if self.phase==1:
            return random.choice([B3_RAIN,B3_RAIN,B3_BLINK,B3_IDLE,B3_RING])
        return random.choice([B3_RAIN,B3_RAIN,B3_BLINK,B3_BLINK,B3_IDLE,B3_RING,B3_CLONE])
...
class Boss4(Boss):
    ...
    def _pick_next(self):
        # LIGHTNING(낙뢰 장판)/DASH(돌진) 비중↑, BARRAGE/THUNDER 탄막 비중↓
        if self.phase==1:
            return random.choice([B4_LIGHTNING,B4_LIGHTNING,B4_DASH,B4_IDLE,B4_BARRAGE])
        return random.choice([B4_LIGHTNING,B4_LIGHTNING,B4_DASH,B4_DASH,B4_IDLE,B4_BARRAGE,B4_THUNDER])
...
class Boss5(Boss):
    ...
    def _pick_next(self,ph):
        # VORTEX(흡입 장판)/METEOR(파도 장판) 비중↑, CHAOS_B 나선 탄막 비중↓
        if ph<=2:
            return random.choice([B5_VORTEX,B5_VORTEX,B5_METEOR,B5_METEOR,B5_IDLE,B5_CHAOS_B])
        if ph==3:
            return random.choice([B5_VORTEX,B5_VORTEX,B5_METEOR,B5_METEOR,B5_IDLE,B5_CHAOS_B,B5_ENRAGE])
        return random.choice([B5_VORTEX,B5_METEOR,B5_ENRAGE,B5_ENRAGE])
