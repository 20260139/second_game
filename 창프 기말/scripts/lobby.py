...
def _load_idle_frames():
    raw   = base64.b64decode(player_base.IDLE_0)
    sheet = pygame.image.load(BytesIO(raw)).convert_alpha()
    frames = []
    for col in range(8):
        rect  = pygame.Rect(col*CELL, 0, CELL, CELL)
        frame = pygame.transform.scale(sheet.subsurface(rect).copy(), (DRAW_SIZE, DRAW_SIZE))
        frames.append(frame)
    return frames
